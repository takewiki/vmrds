...
#shinyserver start point----
 shinyServer(function(input, output,session) {
   ...
   #2.3物料属性批量修改-----
    ...
    #bom查询-------
    var_ERP_BOM_del_FNumber = tsui::var_text('ERP_BOM_del_FNumber')
    observeEvent(input$ERP_BOM_query_action,{
      #code here:
      #新增功能
      FNumber = var_ERP_BOM_del_FNumber()
      print(FNumber)
      if(FNumber==""){
        pop_notice('BOM查询前请输入物料编码:')
      }else{
        #BOM查询
        data = vmrdspkg::erp_bom_version_query(conn_erp = conn_erp,FNumber = FNumber)
        ncount =nrow(data)
        if(ncount){
           tsui::run_dataTable2(id = 'ERP_BOM_VERSION_DT',data = data)
        pop_notice('BOM查询成功')
        }else{
          pop_notice('未查询结果,请确认一下物料编号是否有误')
        }
   ...
   #BOM删除------- = 
    var_ERP_BOM_del_FVersion = tsui::var_text('ERP_BOM_del_FVersion')
    observeEvent(input$ERP_BOM_del_action,{
      
      flag_toCheck = input$erp_bom_check
      #pop_notice(flag_toCheck)
      FNumber = var_ERP_BOM_del_FNumber()
      FVersion = var_ERP_BOM_del_FVersion()
      logi_mo = vmrdspkg::erp_bom_existInMo(conn_erp = conn_erp,FNumber = FNumber,FVersion_PLM = FVersion)
      if(flag_toCheck){
        #检查一下BOM是否在生产任务单中引入
        if(logi_mo){
          #BOM在生产任务单中已存在
          #DO NOT CODING HERE
          pop_notice('BOM在生产任务中已存在,不能执行删除操作')
        }else{
          vmrdspkg::erp_bom_version_delete(conn_erp = conn_erp,FNumber =FNumber,FVersion_PLM =FVersion  )
          pop_notice("BOM在生产任务中不存在,可以执行删除操作")
          #执行删除操作，可以恢复一次
        ...
        if(logi_mo){
          #BOM在生产任务单中已存在
          vmrdspkg::erp_bom_version_delete(conn_erp = conn_erp,FNumber =FNumber,FVersion_PLM =FVersion)
          pop_notice('BOM在生产任务中已存在,强制执行删除')
          #执行删除操作，可以恢复一次
    ...
    #BOM恢复-----
    observeEvent(input$ERP_BOM_recover_action,{
      #BOM恢复操作
      #设置变量
      FNumber = var_ERP_BOM_del_FNumber()
      FVersion = var_ERP_BOM_del_FVersion()
      #A提前检验
      flag_last = vmrdspkg::erp_bom_existInLast(conn_erp = conn_erp,FNumber =FNumber,FVersion_PLM = FVersion )
      if(flag_last){
        #如果已经存在，进入下步操作
        flag_bom = vmrdspkg::erp_bom_existInBom(conn_erp = conn_erp,FNumber =FNumber,FVersion_PLM = FVersion)
        if(flag_bom){
          #BOM已存在，不需要进行恢复
          tsui::pop_notice('BOM已存在，不需要进行恢复')
        }else{
          #BOM不存在，执行恢复
          vmrdspkg::erp_bom_version_recover(conn_erp = conn_erp,FNumber =FNumber,FVersion_PLM = FVersion)
          tsui::pop_notice('执行恢复成功')
        }
      }else{
        #如果不存在，提供不能恢复
        pop_notice('没有最近一次可供恢复的BOM资料，操作已取消!')
      }
      
      
    })
    #BOM降级-----
    var_ERP_BOM_degrade_FNumber = tsui::var_text('ERP_BOM_degrade_FNumber')
    var_ERP_BOM_degrade_FBomNumber = tsui::var_text('ERP_BOM_degrade_FBomNumber')
    var_ERP_BOM_degrade_FVersion_from = tsui::var_text('ERP_BOM_degrade_FVersion_from')
    var_ERP_BOM_degrade_FVersion_to =  tsui::var_text('ERP_BOM_degrade_FVersion_to')
    var_ERP_BOM_degrade_days = tsui::var_text('ERP_BOM_degrade_days')
    observeEvent(input$ERP_BOM_degrade_action,{
      #变量获取
      FNumber = var_ERP_BOM_degrade_FNumber()
      FBomNumber = var_ERP_BOM_degrade_FBomNumber()
      FVersion_from = var_ERP_BOM_degrade_FVersion_from()
      FVersion_to = var_ERP_BOM_degrade_FVersion_to()
      days = as.integer(var_ERP_BOM_degrade_days())
      #测试字段
      if(FNumber==''| FBomNumber=='' | FVersion_from=='' |FVersion_to==''){
        tsui::pop_notice('一个或多个输入参数为空，请检查')
        
      }else{
        if(FVersion_from > FVersion_to){
          #从高版本到低版本
          #检验中间表的低版本
          flag_low = vmrdspkg::erp_bom_degrade_checkExistsInPLM(conn_erp = conn_erp,FNumber = FNumber,FBomNumber = FBomNumber,FVersion_PLM = FVersion_to,days = days)
          if(flag_low){
            #如果中间表存在，继续处理
            flag_high <-vmrdspkg::erp_bom_existInBom(conn_erp = conn_erp,FNumber = FNumber,FVersion_PLM =FVersion_from )
            if(flag_high){
              #ERP系统存在，请继续处理
              #删除ERP系统数据
              vmrdspkg::erp_bom_version_delete(conn_erp = conn_erp,FNumber =FNumber,FVersion_PLM =FVersion_from)
              #重置中间表数据状态
              vmrdspkg::erp_bom_degrade_resetStatusInPLM(conn_erp = conn_erp,FNumber = FNumber,FBomNumber = FBomNumber,FVersion_PLM = FVersion_to,days = days)
              
              tsui::pop_notice('BOM已降版，5分钟后BOM将完成数据同步')
            }else{
              #ERP系统不存在，请联系管理员检查ERP系统BOM数据
              tsui::pop_notice('ERP系统不存在，请联系管理员检查ERP系统BOM数据')
            }
          }else{
            #中间表不存在，请检验PLM数据
            tsui::pop_notice('中间表不存在，请联系管理员检查TC系统或中间表数据!')
          }
        }else{
          #版本相同，不需要进行降版处理
          tsui::pop_notice('版本号没有从高到低，不需要进行降版处理')
